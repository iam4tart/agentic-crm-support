...
class WorkflowNodes:
    ...
    def execute_steps_node(self, state: GraphState) -> dict:
        ...
        for raw_step in state["reasoning_steps"]:
            step = raw_step.get("tool_call", "") if isinstance(raw_step, dict) else str(raw_step)
            step = step.lower()
            ...
            if "retrieve" in step:
                ...
                try:
                    already_created = any(out.get("type") == "jira_create" for out in tool_outputs)
                    
                    if already_created:
                        logger.info("Jira ticket already exists in state, skipping duplicate creation.")
                    elif "create" in state["user_query"].lower() or "ticket" in state["user_query"].lower():
                        issue = self.jira.create_issue(
                            summary=f"Support Request: {state['user_query'][:50]}",
                            description=f"Automated ticket created for: {state['user_query']}",
                            project_key=settings.JIRA_PROJECT_KEY
                        )
                        tool_outputs.append({"type": "jira_create", "data": issue})
                    else:
    ...
    def should_continue(self, state: GraphState) -> str:
        ...
        if score >= 0.7 or retries >= settings.MAX_RETRIES:
            logger.info(f"Ending workflow. Score: {score}, Retries: {retries}")
            return "end"
        
        logger.info(f"Refining answer... Score: {score}, Retries: {retries}")
        return "continue"
