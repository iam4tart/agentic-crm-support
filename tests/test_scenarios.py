...
def test_workflow_initialization(mock_settings):
    with patch("orchestrator.workflow.Retriever"), patch("orchestrator.workflow.Reasoner"):
...
def test_jira_get_issue(mock_settings):
    ...
    with patch("requests.get") as mock_get:
        mock_resp = MagicMock()
        mock_resp.json.return_value = {"key": "TEST-1", "fields": {"summary": "Mock Issue"}}
...
def test_reasoning_plan_generation(mock_settings):
    with patch("agents.reasoning.pipeline") as mock_pipeline:
        ...
        mock_generator.return_value = [{"generated_text": '{"steps": ["retrieve", "tool_call", "respond"]}'}]
...
def test_workflow_plan_node(mock_settings):
    with patch("orchestrator.workflow.Retriever"), patch("orchestrator.workflow.Reasoner") as MockReasoner:
        mock_reasoner_instance = MockReasoner.return_value
        mock_reasoner_instance.generate_plan.return_value = {"steps": ["retrieve"]}
