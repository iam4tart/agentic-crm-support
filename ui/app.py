...
with gr.Blocks(title="Support Ticket Resolution") as demo:
    with gr.Column(elem_classes="container"):
        ...
        gr.Markdown("Autonomous agentic workflow integrating local RAG, LangGraph, and JIRA.", elem_classes="sub-text")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                query_input = gr.Textbox(
                    ...
                    placeholder="Enter the customer issue or request here...", 
                    lines=4
                )
                
                gr.Examples(
                    examples=[
                        ["Login failed with incorrect password error."],
                        ["Checkout page throwing 500 server error."],
                ...
                score_output = gr.Textbox(label="Ragas Combined Score", interactive=False)
            ...
            with gr.Column(scale=2):
                ...
                with gr.Tabs():
                    ...
                    with gr.TabItem("Tool Invocations"):
                        tools_output = gr.Code(language="json", label="JIRA API Payloads")

    submit_btn.click(
        fn=process_ticket,
        inputs=query_input,
        outputs=[answer_output, reasoning_output, knowledge_output, tools_output, score_output]
    )

if __name__ == "__main__":
    demo.launch(server_name="0.0.0.0", server_port=8501, share=False, theme=theme, css=css)
