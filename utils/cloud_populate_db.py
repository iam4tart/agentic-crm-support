...
class CloudDatabasePopulator:
    ...
    def __init__(self):
        ...
        self.client = chromadb.HttpClient(host='https://api.trychroma.com', tenant=settings.chroma_tenant, database=settings.chroma_database, headers={'X-Chroma-Token': settings.chroma_api_key})
    ...
    def normalize_row(self, row, source):
        ...
        def extract(candidates, default='Unknown'):
            ...
            return default
        return {'title': extract(title_candidates, 'No Title'), 'issue': extract(issue_candidates, 'No Issue Description'), 'category': extract(category_candidates, 'Uncategorized'), 'priority': extract(priority_candidates, 'Normal'), 'resolution': extract(resolution_candidates, 'No Resolution'), 'source': source}
    ...
    def embed_and_store(self, records):
        ...
        for record in records:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if doc_id not in seen_ids:
                    seen_ids.add(doc_id)
                    docs.append(chunk)
                    metas.append({'title': record['title'], 'category': record['category'], 'priority': record['priority'], 'source': record['source']})
                    ids.append(doc_id)
        if not ids:
            return
        for i in range(0, len(docs), self.batch_size):
            ...
            self.collection.add(documents=b_docs, metadatas=b_metas, embeddings=embeddings, ids=b_ids)
